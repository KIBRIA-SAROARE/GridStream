...
void fir_filter_step(
    ...
    buffer_real[0] = demod_real;
    buffer_imag[0] = demod_imag;
    ...
    result[0] = (demod_t)acc_real;
    result[1] = (demod_t)acc_imag;
}
...
void pmu_top(
    ...
    // 2. FIR Filter
    demod_t fir_res[2];
    fir_filter_step(demod_real, demod_imag, fir_coeffs, fir_buffer_real, fir_buffer_imag, fir_res);
    demod_t filt_real = fir_res[0];
    demod_t filt_imag = fir_res[1];

    // 3. CORDIC
    mag_t cordic_res[2]; // [0]=mag, [1]=angle
    cordic_kernel<demod_t, mag_t>(filt_real, filt_imag, cordic_res);
    mag_t magnitude = cordic_res[0];
    angle_t angle = (angle_t)cordic_res[1];
    ...
    // 6. LSE
    lse_t diff = (lse_t)aligned_raw_val - (lse_t)est_voltage;
    lse_t lse = diff * diff;
